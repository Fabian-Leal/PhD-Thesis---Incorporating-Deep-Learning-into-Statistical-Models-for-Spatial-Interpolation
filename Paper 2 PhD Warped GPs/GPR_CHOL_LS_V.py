...
class GaussianProcessRegressionLengthscaleVarianceCholesky:
    def __init__(self, cov_function_name, x_train, y_train, hyper_params, sigma_n, warp_params=None, nu=None):
        ...
        self.L = cho_factor(self.K + np.eye(self.K.shape[0]) * (self.sigma_n ** 2), lower = True, check_finite=False)
    ...
    def set_hyper_params(self, hyper_params, sigma_n, warp_params=None):
        ...
        self.K = self.cov_func(self.x_train, self.x_train) + np.eye(self.K.shape[0]) * (self.sigma_n ** 2)
        self.sigma_n = sigma_n
        self.L = cho_factor(self.K, lower = True, check_finite=False)
    ...
    def log_marginal_likelihood(self):
        ...
        lml4 = np.sum(np.log(self.df_dy + 1e-8))
    ...
    def fit(self):
        if self.warp_params is None:
            ...
            self.set_hyper_params([1,self.optimal_params[0]],self.optimal_params[1])
        else:
            def obj_func(params):
                params = np.exp(params)
            ...
            self.set_hyper_params([1,self.optimal_params[0]], self.optimal_params[1], [a,b,c])
